...
(function () {
  ...
  var TRUCK_KEY = 'webtrack_sync_truck_v1';
  ...
  function readTruckInfo() {
    try {
      var raw = localStorage.getItem(TRUCK_KEY);
  ...
  function writeTruckInfo(info) {
    try {
      localStorage.setItem(TRUCK_KEY, JSON.stringify(info || null));
    } catch (e) {}
  }
  ...
  window.addEventListener('storage', function (e) {
    if (e.key === JOBS_KEY || e.key === TRUCK_KEY) notifyListeners();
  });
  ...
  window.WebtrackSync = {
    ...
    // Which real truck/driver Job Composer currently has mirrored to the
    // Driver App — e.g. {name, driver, primeMover, combination} — so the
    // Driver App's header can show the actual truck instead of a fixed
    // placeholder. Set alongside setJobs whenever the live truck changes.
    setTruckInfo: function (info) {
      writeTruckInfo(info);
      broadcastChange();
      notifyListeners();
    },
    ...
    updateJobStatus: function (jobId, patch) {
      ...
      if (patch && patch.arrivedAt) {
        job.arrivedAt = Object.assign({}, job.arrivedAt, patch.arrivedAt);
      }
      writeJobs(jobsArray);
      broadcastChange();
      notifyListeners();
